...
export async function minimalCompiler(tsconfig: string, extraOptions: ts.CompilerOptions = {}) {
    ...
    const program = ts.createProgram(parsedConfigFile.raw.files, parsedConfigFile.options);
    ...
    if (result.emitSkipped) throw new Error('Compilation Error');
}